...
if (isset($_POST["btnAdd"])) {
    ...
    $sql = "INSERT INTO Car (CarNumber, Type, PerDayPrice, RentalID, Image) 
            VALUES ('$CarNum', '$Type', '$Price', '$RentalID', '$imageFileName')";

    if (mysqli_query($con, $sql)) {
        echo "<script>alert('Vehicle added successfully!'); window.location.href='ManageCar.php';</script>";
    } else {
...
if (isset($_POST["btnUpdate"])) {
    ...
    $sql = "UPDATE Car 
            ...
            WHERE CarNumber = '$CarNum' AND RentalID = '$RentalID'";

    if (mysqli_query($con, $sql)) {
        echo "<script>alert('Vehicle updated successfully!'); window.location.href='ManageCar.php';</script>";
    } else {
        echo "Error: " . mysqli_error($con);
    }
}
...
if (isset($_POST["btnRemove"])) {
    ...
    if (mysqli_query($con, $sql)) {
        echo "<script>alert('Vehicle deleted successfully!'); window.location.href='ManageCar.php';</script>";
    } else {
        echo "Error: " . mysqli_error($con);
    }
}

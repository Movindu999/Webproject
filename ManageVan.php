...
$sql = "SELECT VanNumber, Type, PerDayPrice, Image FROM Van WHERE RentalID = '$RentalID'";
$result = mysqli_query($conn, $sql);
?>
<!DOCTYPE html>
<html>
<head>
    <title>Manage Vans - RentOnGo</title>
...
<div class="container mt-5">
    <h2 class="mb-4">Manage Vans</h2>

    <form action="AddVan.php" method="POST" enctype="multipart/form-data" class="mb-5 p-4 bg-white shadow rounded">
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="text" name="txtVanNum" id="txtVanNum" class="form-control" required>
            </div>
            <div class="col-md-4 mb-3">
                <label>Type</label>
        ...
        <button type="submit" class="btn btn-success" name="btnAdd">Add Vehicle</button>
        <button type="submit" class="btn btn-danger" name="btnRemove">Remove Vehicle</button>
        <button type="submit" class="btn btn-primary" name="btnUpdate">Update Vehicle</button>
    </form>
        ...
        <tbody>
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($row['VanNumber']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['Type']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['PerDayPrice']) . "</td>";
                    echo "<td data-img='" . htmlspecialchars($row['Image']) . "'><img src='uploads/" . htmlspecialchars($row['Image']) . "' alt='Vehicle Image' width='100'></td>";
                    echo "</tr>";
                }
            } else {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        for (let i = 1; i < rows.length; i++) {
            rows[i].addEventListener("click", function () {
                ...
                if (cells.length >= 4) {
                    document.getElementById("txtVanNum").value = cells[0].innerText;

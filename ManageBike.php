...
$sql = "SELECT BikeNumber, Type, PerDayPrice, Image FROM Bike WHERE RentalID = '$RentalID'";
...
<div class="container mt-5">
    ...
    <form action="AddBike.php" method="POST" enctype="multipart/form-data" class="mb-5 p-4 bg-white shadow rounded">
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="text" name="txtBikeNum" id="txtBikeNum" class="form-control" required>
            </div>
            <div class="col-md-4 mb-3">
                <label>Type</label>
        ...
        <tbody>
            ...
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($row['BikeNumber']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['Type']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['PerDayPrice']) . "</td>";
                    echo "<td>" . (!empty($row['Image']) ? '<img src="uploads/' . htmlspecialchars($row['Image']) . '" alt="Vehicle Image" style="max-width: 100px;">' : 'No Image') . "</td>";
                    echo "</tr>";
                }
            } else {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        for (let i = 1; i < rows.length; i++) {
            rows[i].addEventListener("click", function () {
                ...
                if (cells.length >= 4) {
                    document.getElementById("txtBikeNum").value = cells[0].innerText;
